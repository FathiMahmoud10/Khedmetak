...
namespace Khedmetak.API.Controllers
{
    ...
    public class AdminStepsController : ControllerBase
    {
        ...
        public AdminStepsController(IGovServiceAdminService adminService) => _adminService = adminService;
        ...
        public async Task<IActionResult> AddStep(int id, [FromBody] CreateServiceStepDto dto)
        {
            ...
            if (step is null) return NotFound(ApiResponse<object>.Fail($"Service with id {id} not found."));
        ...
        public async Task<IActionResult> UpdateStep(int id, int stepId, [FromBody] UpdateServiceStepDto dto)
        {
            ...
            if (step is null) return NotFound(ApiResponse<object>.Fail($"Step with id {stepId} not found for service {id}."));
        ...
        public async Task<IActionResult> DeleteStep(int id, int stepId)
        {
            ...
            if (!deleted) return NotFound(ApiResponse<object>.Fail($"Step with id {stepId} not found for service {id}."));
