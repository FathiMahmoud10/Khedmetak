...
namespace Khedmetak.API.Controllers
{
    ...
    public class AdminRequiredDocumentsController : ControllerBase
    {
        ...
        public AdminRequiredDocumentsController(IGovServiceAdminService adminService)
        {
        ...
        /*  عرض كل المستندات المطلوبة للخدمة */
        ...
        /*  إضافة مستند مطلوب جديد للخدمة (اسم / إلزامي أو لا / نوع الملف) */
        ...
        /*  تعديل مستند مطلوب (الاسم / الإلزامية / النوع) */
        ...
        public async Task<IActionResult> UpdateRequiredDocument(int id, int docId, [FromBody] UpdateRequiredDocumentDto dto)
        {
            ...
            if (doc is null)
                return NotFound(ApiResponse<object>.Fail($"Document with id {docId} not found for service {id}."));

            return Ok(ApiResponse<object>.Ok(doc, "Required document updated successfully."));
        }

        /*  حذف مستند مطلوب من الخدمة */
